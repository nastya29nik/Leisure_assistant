document.addEventListener('DOMContentLoaded', function() {
    ...
    let imagePath = place.image || '/static/images/default-place.jpg';
   ...
   function getCsrfToken() {
    ...
    try {
        ...
        const modalHtml = `
            <div class="place-detail-modal" data-id="${place.id}">
                <img src="${place.image || '/static/images/default-place.jpg'}" 
                     class="place-image"
                     onerror="this.src='/static/images/default-place.jpg'">
    ...
    // Добавляем сообщение о загрузке
        addMessage(`<div class="summary-loading">Загружаю сводку для "${placeName}"<span class="loading-dots"><span>.</span><span>.</span><span>.</span></span></div>`, 
                false, false, true);
        ...
        try {
            ...
            if (summaryData.summary) {
                summaryHtml += `<div class="summary-content">${summaryData.summary}</div>`;
            } else if (summaryData.error) {
                summaryHtml += `<div class="summary-error">${summaryData.error}</div>`;
            } else {
                summaryHtml += `<div class="summary-error">Не удалось получить сводку</div>`;
            }
        ...
        // Заменяем сообщение о загрузке на результат
            ...
            console.error('Ошибка:', error);
            const errorMessage = `<div class="summary-error">Ошибка при загрузке сводки: ${error.message}</div>`;
            const messages = document.querySelectorAll('.message');
            const lastMessage = messages[messages.length - 1];
